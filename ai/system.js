...
function formatAPIError(err) {
  ...
  // Axios 或 fetch 的 HTTP 狀態碼
  const status = err.response?.status;

  switch (status) {
    case 400:
      errorMessage = '請求格式錯誤 (Bad Request)';
      break;
    case 401:
      errorMessage = 'API Key 錯誤或未授權 (Unauthorized)';
      break;
    case 403:
      errorMessage = '權限不足或被封鎖 (Forbidden)';
      break;
    case 404:
      errorMessage = '資源不存在 (Not Found)';
      break;
    case 408:
      errorMessage = '連線逾時 (Request Timeout)';
      break;
    case 429:
      errorMessage = '超出 API 配額 / 請求太頻繁 (Too Many Requests)';
      break;
    case 500:
      errorMessage = '伺服器內部錯誤 (Internal Server Error)';
      break;
    case 502:
      errorMessage = 'Bad Gateway';
      break;
    case 503:
      errorMessage = '服務暫時不可用 (Service Unavailable)';
      break;
    case 504:
      errorMessage = '伺服器回覆逾時 (Gateway Timeout)';
      break;
    default:
      errorMessage = err.message || '其他錯誤';
      break;
  }
...
async function pingModelEntry(entry) {
  try {
    ...
    console.warn(`⚠️ API Ping 測試失敗：${err.message}`);
    return false;
  }
}
...
// 短期記憶設定（只存在記憶體，重啟後消失）
const SHORT_TERM_LIMIT = 30; // 最多 30 句短期記憶
const shortTermMemory = new Map(); // userId -> [{role:'user'|'bot', text, ts}]

// 新增短期記憶（會以隊列方式維持最新 N 筆）
...
// 現在合併檔案記憶與短期記憶，優先列出檔案記憶（family/bot + user）
function getShortTermMemoryLines(userId) {
  const arr = shortTermMemory.get(userId) || [];
  const shortLines = arr.map((m) => `${m.role}: ${m.text}`);

  // 從快取取得檔案記憶（若快取沒有則嘗試載入）
  let userFileData = [];
  try {
    if (userMemoryCache.has(userId)) {
      userFileData = userMemoryCache.get(userId).data || [];
    } else {
      userFileData = loadUserMemoryToCache(userId) || [];
    }
  } catch (e) {
    userFileData = [];
  }
  ...
  let familyData = [];
  try {
    if (botMemoryCache && Array.isArray(botMemoryCache.data)) {
      familyData = botMemoryCache.data;
    } else {
      familyData = loadBotMemoryToCache() || [];
    }
  } catch (e) {
    familyData = [];
  }

  const fileLines = [...familyData, ...userFileData].map((item) => {
    if (typeof item === 'string') return item;
    try { return JSON.stringify(item); } catch (e) { return String(item); }
  });

  // 組合：檔案記憶先，短期記憶後
  return [...fileLines, ...shortLines];
}
...
async function handleAIMessage(userId, prompt) {
  ...
  // 原始 system prompt 內容（部分已移除違規文字以符合法規與平台政策）
...
當前模型是: **${aimodel}**`
  ...
  try {
    ...
    // 取得短期記憶（最新） - 這裡會包含檔案記憶 + 短期記憶
    const shortLines = getShortTermMemoryLines(userId);
    const shortText = shortLines.length ? `短期與檔案記憶:\n${shortLines.join('\n')}` : '';
    ...
    try {
      ...
      console.warn('⚠️ 解析模型回覆失敗：', e?.message || e);
      responseText = '# 😴 我睡著了，這是錯誤細節：\n```bash\n❌ 內容不可用或被過濾，再試一次吧\n```';
    }
